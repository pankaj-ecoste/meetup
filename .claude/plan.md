...
**Note:** once §8.13 lands, this must also cover **iOS standalone (installed) mode**, not just Safari — `getUserMedia` behaves differently there and a break would take out the core recording flow.
...
### 8.13 Installable PWA shell — ✅ **BUILT 2026-07-31** (⚠️ not yet verified on real hardware)
**What:** a web app manifest, real icons, standalone display mode, and the legacy Apple meta tags, so MeetUp installs to the home screen on both Android and iOS.
**Why:** at pilot scale, URL friction is a rounding error. At ~100 tasks/day an individual opens the app 5–10× daily to check received tasks, mark complete, and approve extensions — friction per open multiplies across ~300 people. Employees on a factory or site floor will tap an icon; they will not hunt a bookmark. This is an **adoption** risk, not a technical one, and it can make the pilot look worse than the extraction accuracy actually is.
**Where:** `app/manifest.ts` (Next 16 metadata route); real icons in `public/` (currently only Next's default SVGs); `apple-touch-icon` + `apple-mobile-web-app-capable` in `app/layout.tsx` metadata — **the manifest alone gives a half-broken result on iOS** (generic screenshot instead of the icon).
**Watch out:**
- **iOS standalone mode uses a storage container separate from Safari** — a session in Safari does not carry into the installed app. Sequence onboarding as *install first, then `/claim` inside the installed app*, never the reverse.
- **`getUserMedia` in iOS standalone must be verified on real hardware** before this ships — if standalone breaks the mic, it breaks the core feature. Test alongside §8.6, not separately.
**Done when:** the app installs to the home screen with a correct icon and no address bar on a real Android and a real iPhone, and all three recording flows still complete from the installed app.

**As built:**
- `app/manifest.ts` — `display: 'standalone'`, `scope: '/'`, portrait, `theme_color` indigo-600, `background_color` gray-50 to match the app shell. Next auto-injects `<link rel="manifest">` from the file convention, so `metadata.manifest` is deliberately **not** set (it would emit a duplicate tag).
- `public/icon-192.png`, `public/icon-512.png` (`purpose: 'any'`) and `public/icon-maskable-512.png` (`purpose: 'maskable'`, glyph scaled to 0.66 so launcher circle-cropping does not clip it). `app/apple-icon.png` (180×180, full-bleed) via the file convention → `<link rel="apple-touch-icon">`. White mic glyph on indigo-600, matching `RecordButton`.
- `app/layout.tsx` — `appleWebApp` for the iOS title + status-bar style, plus `other: { 'apple-mobile-web-app-capable': 'yes' }` because **Next only emits the modern `mobile-web-app-capable`** and older iOS still reads the Apple-prefixed name. `viewport.themeColor` set; `maximumScale`/`userScalable` deliberately left alone (blocking pinch-zoom in standalone leaves no browser chrome to escape to), and `viewportFit: 'cover'` deliberately **not** set (no safe-area insets in the layout yet, so cover would slide the Nav under the iPhone notch).
- `proxy.ts` — **`.webmanifest` added to the matcher's exclusion list.** It was previously being caught by the auth redirect: an unauthenticated `GET /manifest.webmanifest` returned a 307 to `/login`, which makes the manifest unparseable and silently kills the install prompt on the one screen a new employee sees first. Verified after the fix: manifest and both icons return 200 without cookies, while `/performance` still 307s to `/login`.
- Verified: `tsc --noEmit` clean, `npm run build` clean, lint at the pre-existing baseline (2 errors / 3 warnings, all in `ScoreBands`/`TaskRegister` and three pages, none in touched files). All eight head tags confirmed emitted exactly once in the prerendered `/login` HTML.

**Still open:** no install-discovery hint in the UI. Android fires its own install prompt, but **iOS has no prompt API at all** — a user must know to tap Share → Add to Home Screen, so iPhone install is currently discoverable only by being told. Verification on physical devices is §8.6.

### 8.14 Web push notifications — ⚠️ **PARTIALLY BUILT 2026-07-31** (migration + VAPID keys NOT yet applied to production)
**What:** a service worker, VAPID keys, a `push_subscriptions` table, and a send path, wired to task-assigned / deadline-due / extension-decided events.
**Why:** §8.7's toast only fires while the app is open, which is useless for telling someone about a task they don't know exists. At ~100 tasks/day the v2 WhatsApp plan means roughly 300–400 messages/day (~10k/month) at Meta's per-conversation rate, recurring forever and growing with adoption. **Web push is free and unmetered**, so the right architecture is push where the device supports it, WhatsApp as fallback — push becomes the cost-control layer, not a redundant second channel.
**Where:** new `public/sw.js`; new `push_subscriptions` table + RLS; a send helper in `lib/server/`; hooks in `POST /api/tasks/batch` and the extension-decide route.
**Watch out:** **iOS delivers push only after the user has manually installed to the home screen** (16.4+), and it fails *silently* — no error, no notification. On a mixed Android/iPhone fleet this means a systematic invisible gap, and at 100 tasks/day it is too high-volume to catch by hand. Therefore: **track per-user subscription state and fall back to WhatsApp for anyone without a live subscription.** "Push and hope" is not acceptable here. Depends on §8.13 (install is a precondition on iOS).
**Done when:** assigning a task push-notifies a subscribed device with the app closed, and a user with no live subscription is provably routed to the WhatsApp fallback instead. The fallback half lands with the §9 v2 WhatsApp work; the push half can ship before it.

**As built:**
- `supabase/migrations/0021_push_subscriptions.sql` — new table (`user_id`, `endpoint` unique, `p256dh`, `auth`), RLS scoped to the owning user, same `auth_id`-join convention as every other policy in `setup.sql`. **Not yet run against production** — same manual-migration workflow as 0019/0020; push is fully inert until this runs (see below).
- `lib/server/webpush.ts` — `sendPushToUser(userId, payload)`. Reads VAPID keys from env once; if either key is unset, every call is a silent no-op. Sends to every subscription row for that user, deletes a subscription on a 404/410 response (device unsubscribed / uninstalled), and **never throws** — errors are logged and swallowed so a notification failure can never affect the caller's response.
- `app/api/push/subscribe/route.ts` — `POST` upserts on `endpoint` (re-subscribing after clearing site data replaces, doesn't duplicate), `DELETE` removes one. Both `requireUser`-gated like every other route.
- `public/sw.js` — push + notificationclick only, **deliberately no `fetch` handler** — nothing is cached, so it cannot make any existing page or API response stale. Matches the "no offline mode, every flow needs the network" conclusion from the earlier PWA design discussion.
- `components/PushOptIn.tsx` — dismissible opt-in banner on the Dashboard, **requires an explicit tap, never auto-prompts.** A denied browser permission can't be re-asked by the page, so the one shot at a "yes" is spent on a moment the user chose. Dismissal is remembered in `localStorage`.
- Hooked into exactly the two routes named above in **Where**: `POST /api/tasks/batch` (notifies each assignee) and `PATCH /api/extensions/[id]/decide` (notifies the requester — the task's assignee, not the assignor — hence the deep-link target is `/received`, not `/allocated`). **Deadline-due is NOT hooked** — that depends on the §8.1 overdue-transition job, which doesn't exist yet, so it was left out rather than half-built against a job that isn't there.
- `NEXT_PUBLIC_VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` added to `.env.example` as placeholders only — a real key pair was generated during this build and given to the user directly (not committed, not written to any file in the repo) for them to add to `.env.local` and Vercel, same handling as every other secret in this project.
- **Verified safe to deploy before the migration or keys exist**: with no VAPID keys set, `sendPushToUser` no-ops before touching the DB. Even if it did query, Supabase's JS client returns `{data: null, error}` rather than throwing when a table doesn't exist, so a pre-migration deploy degrades to "push silently does nothing" — task creation and extension decisions are provably unaffected either way (confirmed against a running build: `/api/tasks/batch` and the auth boundary on other routes behave identically to before this change).
- **Caveat, not fixed:** an unauthenticated request to `/sw.js` gets redirected to `/login` by the existing middleware (same as any other page — `.js` isn't in the static-asset exclusion list, unlike `.webmanifest`). Not a functional bug: `register('/sw.js')` only ever runs from the Dashboard, behind the auth-gated layout, where a session cookie is always present. The only theoretical edge is the browser's own periodic background revalidation of an already-installed worker firing after a session has expired — that fails closed (browser keeps the old cached worker), never surfaces to the user. Left as-is rather than widening the middleware's public-asset exclusion without a demonstrated need.

**Still open:** the WhatsApp-fallback tracking this entry's Done-when calls for isn't built — it depends on the v2 WhatsApp integration, which is still pilot-gated. Right now a user with no push subscription simply gets nothing; there is no fallback channel yet.
