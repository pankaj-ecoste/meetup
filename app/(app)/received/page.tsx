...
export default function ReceivedPage() {
  ...
  const fetchTasks = useCallback(async (tok: string, q: string, p: number) => {
    setLoading(true)
    try {
      const data: TaskResponse[] = await api.tasksReceived(tok, p, q)
      setTasks(data)
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6">
      <h2 className="text-xl font-bold text-gray-900 mb-5">Tasks Received</h2>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-gray-400 text-sm">
          {search ? 'No tasks match your search.' : 'No tasks assigned to you yet.'}
        </div>
      ) : (
