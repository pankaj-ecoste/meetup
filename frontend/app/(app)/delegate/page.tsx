...
export default function DelegatePage() {
  ...
  async function handleRecordingComplete(blob: Blob, filename: string) {
    ...
    try {
      const job = await api.uploadRecording(token, blob, 'task_delegation', filename)
      setStage('processing')
      ...
      // Fallback: poll once after 60s if Realtime doesn't fire
      setTimeout(async () => {
        if (stage !== 'review' && stage !== 'error') {
          try {
            const j = await api.me(token) // just to confirm session alive
            void j
          } catch { /* ignore */ }
        }
      }, 60000)
    ...
    <div className="max-w-lg mx-auto px-4 py-6">
      ...
      {(stage === 'uploading' || stage === 'processing') && (
        <div className="flex flex-col items-center py-12 gap-4">
          ...
          <p className="text-gray-400 text-xs">This takes 15–30 seconds</p>
        </div>
      )}
